...
type Props = {
  ...
  maxMonto: number;
  pago?: ProviderPayment | null;
};

const { quotationProviderId, maxMonto, pago = null } = defineProps<Props>();

const emit = defineEmits<{
  submit: [data: ProviderPaymentFormData];
  cancel: [];
}>();
...
function onSubmit() {
  ...
  const data: ProviderPaymentFormData = {
    ...result.data,
    quotationProviderId,
    ...(pago?.id ? { id: pago.id } : {}),
  };
