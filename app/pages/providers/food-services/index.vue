...
const categoryProviders = computed(() => providerStore.getProvidersByCategory(PROVIDER_CATEGORY.FOOD_SERVICES));
...
async function handleFormSubmit(data: ProviderFormData) {
  try {
    data.category = PROVIDER_CATEGORY.FOOD_SERVICES;
...
const columns: TableColumn<Provider>[] = [
  {
    ...
    cell: ({ row }) =>
      h('div', { class: 'flex items-center gap-2' }, [
        h('span', { class: 'i-lucide-utensils w-4 h-4 text-gray-400' }),
  ...
  <div class="space-y-6">
    <div class="flex justify-between items-center">
      <div class="flex items-center gap-4">
        <UIcon name="i-lucide-utensils" class="w-10 h-10 text-amber-500" />
        <div>
    ...
    <UCard>
      <div v-if="providers.length === 0" class="text-center py-12">
        <UIcon name="i-lucide-utensils" class="w-16 h-16 text-gray-300 dark:text-gray-600 mx-auto mb-4" />
        ...
        <template v-else>
          <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-2">
            No hay proveedores de comidas registrados aún
          </h3>
          <p class="text-gray-500 dark:text-gray-400 mb-4">
            Comienza agregando tu primer proveedor de comidas
          </p>
    ...
    <UModal
      ...
      :title="editingProvider ? 'Editar Proveedor' : 'Nuevo Proveedor de Comidas'"
    ...
    >
      <template #body>
        <ProviderForm
          ...
          :fixed-categoria="PROVIDER_CATEGORY.FOOD_SERVICES"
