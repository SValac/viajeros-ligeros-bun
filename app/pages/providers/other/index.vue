...
const categoryProviders = computed(() => providerStore.getProvidersByCategory(PROVIDER_CATEGORY.OTHER));
...
async function handleFormSubmit(data: ProviderFormData) {
  try {
    data.category = PROVIDER_CATEGORY.OTHER;
  ...
  <div class="space-y-6">
    ...
    <UCard>
      <div v-if="providers.length === 0" class="text-center py-12">
        <UIcon name="i-lucide-package" class="w-16 h-16 text-gray-300 dark:text-gray-600 mx-auto mb-4" />
        ...
        <template v-else>
          <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-2">
            No hay otros proveedores registrados aún
          </h3>
          <p class="text-gray-500 dark:text-gray-400 mb-4">
