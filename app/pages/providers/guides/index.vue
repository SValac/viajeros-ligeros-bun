...
const categoryProviders = computed(() => providerStore.getProvidersByCategory(PROVIDER_CATEGORY.GUIDES));
...
function handleFormSubmit(data: ProviderFormData) {
  try {
    data.category = PROVIDER_CATEGORY.GUIDES;

    if (editingProvider.value) {
      const success = providerStore.updateProvider(editingProvider.value.id, data);
      if (success) {
        toast.add({ title: 'Guía actualizado', description: `${data.name} se actualizó correctamente`, color: 'primary' });
        closeModal();
      }
    }
    else {
      providerStore.addProvider(data);
      toast.add({ title: 'Guía creado', description: `${data.name} se creó correctamente`, color: 'primary' });
      closeModal();
    }
  }
  catch {
...
function handleDelete(provider: Provider) {
  ...
  if (confirm(`¿Estás seguro de eliminar ${provider.name}?`)) {
    ...
    if (success) {
      toast.add({ title: 'Guía eliminado', description: `${provider.name} se eliminó correctamente`, color: 'warning' });
    }
  }
}
