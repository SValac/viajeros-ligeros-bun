...
const categoryProviders = computed(() => providerStore.getProvidersByCategory(PROVIDER_CATEGORY.GUIAS));
...
function handleFormSubmit(data: ProviderFormData) {
  try {
    data.categoria = PROVIDER_CATEGORY.GUIAS;

    if (editingProvider.value) {
      const success = providerStore.updateProvider(editingProvider.value.id, data);
      if (success) {
        toast.add({ title: 'Guía actualizado', description: `${data.nombre} se actualizó correctamente`, color: 'primary' });
        closeModal();
      }
    }
    else {
      providerStore.addProvider(data);
      toast.add({ title: 'Guía creado', description: `${data.nombre} se creó correctamente`, color: 'primary' });
      closeModal();
    }
  }
  catch {
...
function handleDelete(provider: Provider) {
  ...
  if (confirm(`¿Estás seguro de eliminar ${provider.nombre}?`)) {
    ...
    if (success) {
      toast.add({ title: 'Guía eliminado', description: `${provider.nombre} se eliminó correctamente`, color: 'warning' });
    }
  }
}
...
const columns: TableColumn<Provider>[] = [
  {
    ...
    cell: ({ row }) =>
      h('div', { class: 'flex items-center gap-2' }, [
        h('span', { class: 'i-lucide-user-search w-4 h-4 text-gray-400' }),
  ...
  <div class="space-y-6">
    <div class="flex justify-between items-center">
      <div class="flex items-center gap-4">
        <UIcon name="i-lucide-user-search" class="w-10 h-10 text-blue-500" />
        <div>
    ...
    <UCard>
      <div v-if="providers.length === 0" class="text-center py-12">
        <UIcon name="i-lucide-user-search" class="w-16 h-16 text-gray-300 dark:text-gray-600 mx-auto mb-4" />
        ...
        <template v-else>
          ...
          <p class="text-gray-500 dark:text-gray-400 mb-4">
            Comienza agregando tu primer guía
          </p>
    ...
    <UModal
      ...
      :title="editingProvider ? 'Editar Guía' : 'Nuevo Guía'"
    ...
    >
      <template #body>
        <ProviderForm
          ...
          :fixed-categoria="PROVIDER_CATEGORY.GUIAS"
