...
export function toCoordinatorInviteError(error: unknown): CoordinatorInviteError {
  ...
  if (message === 'not_authorized') {
    return new CoordinatorInviteError('not-authorized', 'No tienes permiso para invitar a este coordinador.', { cause: error });
  }
  ...
  return new CoordinatorInviteError('unknown-error', 'No se pudo completar la invitación del coordinador.', { cause: error });
}
